...
def doWire(grid, cmdsStr, pos):    
    cmds = cmdsStr.split(",")
    counter = 0
    for cmd in cmds:
        if cmd[0]=="L":
            pos, counter = moveH(grid, int(cmd[1:]), -1, pos, counter)  
        elif cmd[0]=="R":
            pos, counter = moveH(grid, int(cmd[1:]), +1, pos, counter)  
        elif cmd[0]=="D":
            pos, counter = moveV(grid, int(cmd[1:]), +1, pos, counter)  
        elif cmd[0]=="U":
            pos, counter = moveV(grid, int(cmd[1:]), -1, pos, counter)  
...
pos = [pos0Y, pos0X]    
grid1 = np.zeros((sizeC, sizeR), int)
doWire(grid1, wire1CmdsStr, pos)

pos = [pos0Y, pos0X]    
grid2 = np.zeros((sizeC, sizeR), int)
doWire(grid2, wire2CmdsStr, pos)

ints =  np.logical_and(grid1, grid2)  #Intersezioni
intsPos = np.nonzero(np.logical_and(grid1, grid2)) #Indici dei valor di ints != 0, ovvero delle intersezioni
distances = abs(intsPos[0]-pos0Y) + abs(intsPos[1]-pos0X) #distanze calcolate solo per le intersezioni
print("minDist: " + str(np.min(distances)))

lengths = grid1[intsPos] + grid2[intsPos]
print("minlength: " + str(np.min(lengths)))

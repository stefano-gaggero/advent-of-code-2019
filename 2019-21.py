...
class JumpRobot:
    ...
    def insertCommand(self, cmd, a, b):
        print(" ".join([cmd, a, b]))
        for c in " ".join([cmd, a, b]):
    ...
    def printOutput(self):
        res = self.prog.getOutputs(True)
        try:
            print(''.join(map(str, [chr(x) for x in res])))
        except:
            print(res[len(res)-1])


robby = JumpRobot()
robby.insertCommand("OR", "A", "T")  #J = true se c'è un buco in A o B o C
robby.insertCommand("AND", "B", "T")
robby.insertCommand("AND", "C", "T")
robby.insertCommand("NOT", "T", "J")

robby.insertCommand("AND", "D", "J")  #In ogni caso salta solo se in D non c'è un buco
robby.walk()
robby.printOutput()  #19359316

#Part 2 -----------------------------------------------------------------------

robby = JumpRobot()
#Logica del salto
# - compare un buco in A o B o C
# - E non c'è un buco né in D
# - E (non c'è un buco in H oppure non c'è buco in E) (se c'è buco in H comunque dopo il salto in D può procedere in E; serve per far fronte a questa situazione: #####.#.##.#.####; non copre comunque tutti i casi)
...
robby.insertCommand("AND", "D", "J")  #J = true solo se c'è buco in A B o C E non cè un buco in D

#Se è comparso un buco in A B o C si ha T=0, se nn c'è buco T=1, ma J=0 e dunque in ogni caso alla fine sarà J=0
robby.insertCommand("OR", "H", "T")  
robby.insertCommand("OR", "E", "T")  
robby.insertCommand("AND", "T", "J")


robby.run()
robby.printOutput()  #1141281622
